    ...
    <div className="bg-white border border-border rounded-2xl shadow-sm overflow-hidden">
      <div className="px-5 py-3 border-b border-border flex items-center justify-between text-sm text-neutral500">
        <div className="flex items-center gap-2">
          <span className="font-semibold">Absent / Pending</span>
        ...
        <table className="w-full table-fixed text-xs">
          <tbody>
            {data.map(row => (
              ...
              >
                <td className="px-5 py-3 w-1/2 text-neutral500">
                  <FormattedDate date={row.label} />
                </td>
                <td className="px-5 py-3 w-1/4 text-neutral400">
                  {row.days} day
                </td>
                <td className="px-5 py-3 w-1/4 text-right space-x-2">
                  {row.isPendingLeave && (
                    <StatusBadge type="INACTIVE" label="Pending leave" />
                  )}
                  {!row.isPendingLeave && (
                    <Button
                      variant="outline"
